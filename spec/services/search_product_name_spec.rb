    ...
    context 'results found' do
      it 'returns product name candidates' do
        VCR.use_cassette 'product_name/search/results_found' do
          expect(described_class.(name: 'iron rangers', name_scraper: AmazonProductNameExtractor)).to eq [
    ...
    context 'result not found' do
      it 'returns error' do
        VCR.use_cassette 'product_name/search/results_not_found' do
          expect(described_class.(name: 'ahsdkjashdkajsdhaksjdhasjkhd', name_scraper: AmazonProductNameExtractor)).to eq []
        end
      end
    end
  end
